...
class CPT(Factor):
    ...
    def __init__(self, data, states=None, conditioned=None, description=''):
        ...
        if len(conditioning):
            ...
            # if np.any(row_sum == 0):
            #     num_dimensions = len(self.values.shape)

            #     # Adjust indexing based on the number of dimensions
            #     if num_dimensions == 3:
            #         self.values[row_sum == 0, :, :] = 0
            #     elif num_dimensions == 2:
            #         self.values[row_sum == 0, :] = 0
            # else:
            #     self.values = self.values / row_sum
            if np.any(row_sum == 0):
                zero_indices = np.argwhere(row_sum == 0)

                # Use boolean indexing to replace values with 0
                indices = tuple(zero_indices.T)
