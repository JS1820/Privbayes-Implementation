...
def preprocess(original_dataset, numeric_bin_size=5):
    ...
    with open(domain_output_file, 'w') as json_file:
        json.dump(domain_correlation_values, json_file)
    print(domain_correlation_values,"_______________correlation values")
    # Save domain correlation values as a JSON file in the output directory
    with open(domain_correlation_file, 'w') as json_file:
        json.dump(domain_values, json_file)
...
def convert_to_original(processed_data, domain_values, file_name):
    for col in processed_data.columns:
        if col in domain_values:
            if isinstance(domain_values[col], int):
                # Categorical values
                ...
                lambda x: np.random.uniform(
                    domain_values[col][str(int(float(x)))]['min'],
                    domain_values[col][str(int(float(x)))]['max']
                ) if str(int(float(x))) in domain_values[col] and isinstance(domain_values[col][str(int(float(x)))], dict) and not pd.isnull(x) else x
...
def postprocess(processed_input_dataset, domain_correlation_file, file_name):
    ...
    #             elif isinstance(domain_values[col], dict):
    #                 # Binned numerical values
    #                 processed_data[col] = processed_data[col].round().astype(int)

    #     return processed_data

    # Convert processed data back to original form
    postprocessed_synthetic_data = convert_to_original(processed_data, domain_values, file_name)
    print(postprocessed_synthetic_data)
    # Save the modified synthetic dataset
    output_directory = '/privbayes-implementation/Privbayes/data/postprocessed-output/'
    if not os.path.exists(output_directory):
        os.makedirs(output_directory)

    modified_synthetic_output_file = os.path.join(output_directory, f'postprocessed_synthetic_{file_name}.csv')
    print(modified_synthetic_output_file)
    postprocessed_synthetic_data.to_csv(modified_synthetic_output_file, index=False)

    print(f'[+] Synthetic Dataset is saved to: {modified_synthetic_output_file}')

    return modified_synthetic_output_file
