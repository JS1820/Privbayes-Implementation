...
def preprocess_dataset(input_data):
    ...
    for column in input_data.columns:
        if pd.api.types.is_numeric_dtype(input_data[column]):
            # Process numerical columns into buckets of size 10
            bucketized_values, mapping = pd.cut(input_data[column], bins=10, labels=False, precision=0, retbins=True)
            # Replace original numerical values with bucketized values in the same column
...
if __name__ == '__main__':
    ...
    data,bucketized_columns = preprocess_dataset(data_original)
    print("\n[+] Head of the 'original' preprocessed dataset is: ")
    print(data.head())
    print("\n[+] Attribute Parent Pairs are being developed ...\n")
    pb = PrivBayes(epsilon=1, n_cpus=6, score_function='R', verbose=True)
    pb.fit(data)
    starttime = time.time()
    # Concatenate the results after all processes are completed
    df_synth_original = pb.sample(40000)
    endtime = time.time()
    print("[+] Time taken to generate 40000 records' synthetic dataset is: ", endtime - starttime,"\n\n")
